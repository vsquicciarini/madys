...
``MADYS`` will compute the required magnitudes, starting from the best-fit solution for the observed photometry of the star. The outputs will be stored as attributes of the resulting ``FitParams`` instance; a "synth\_" prefix is added to the name of the quantity to clarify its synthetic nature. In other words, in the case above the W1 photometry will be stored as ``results.synth_W1``, its 16th/84th percentiles being ``results.synth_W1_min``/``results.synth_W1_max``, respectively.
