...
``SampleObject`` is the class dedicated to catalog queries, computation of extinctions, and parameter estimation. It is here that our (sub)stellar sample of interest and its properties are defined and stored. Virtually any amount of objects (1 to <MATH>\sim 10^5</MATH>) can be provided with a single command, allowing for a wide variety of scientific applications.
